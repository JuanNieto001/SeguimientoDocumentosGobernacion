{{-- Archivo: backend/resources/views/backend/areas/juridica.blade.php | Proposito: Vista documentada para mantenimiento. | @documentado-copilot 2026-04-11 --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <h1 class="text-lg font-bold text-gray-900 leading-none">Mi Bandeja - Juridica</h1>
                <p class="text-xs text-gray-400 mt-1">Procesos en etapa activa de Juridica</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-3 gap-5">
            ...
            <div class="xl:col-span-1 bg-white rounded-2xl overflow-hidden" style="border:1px solid #e2e8f0">
                ...
                <div class="p-3 space-y-2 max-h-[70vh] overflow-y-auto">
                    ...
                    @empty
                    <p class="text-sm text-gray-500 p-3">No hay procesos en Juridica.</p>
                    @endforelse
                </div>
            </div>
            ...
            <div class="xl:col-span-2 bg-white rounded-2xl overflow-hidden" style="border:1px solid #e2e8f0">
                ...
                <div class="p-5 space-y-5">
                    ...
                    @if(!$procesoEtapa)
                    <div class="p-3 rounded-xl text-sm" style="background:#fffbeb;border:1px solid #fde68a;color:#92400e">
                        Este proceso aun no tiene instancia de etapa en Juridica. Marca recibido para crearla y cargar checklist.
                    </div>
                    @endif
                    ...
                    <div>
                        ...
                        @if(!$procesoEtapa)
                        <p class="text-sm text-gray-500">No se puede diligenciar checklist porque no existe instancia de etapa.</p>
                        @else
                        <div class="space-y-2">
                            @forelse($checks as $c)
                            <form method="POST" action="{{ route('workflow.checks.toggle', [$proceso->id, $c->check_id]) }}">
                                ...
                                <input type="hidden" name="area_role" value="{{ $areaRole }}">
                                <button type="submit"
                                        class="w-full text-left px-3 py-2 rounded-xl border text-sm {{ $c->checked ? 'bg-green-50 border-green-200' : 'bg-white border-gray-200' }} {{ !$procesoEtapa->recibido ? 'opacity-50 cursor-not-allowed' : '' }}"
                            ...
                            @empty
                            <p class="text-sm text-gray-500">No hay items de checklist para esta etapa.</p>
                            @endforelse
                        </div>
                        @endif
                    </div>
                    ...
                    <form method="POST" action="{{ route('workflow.enviar', $proceso->id) }}" class="pt-1">
                        ...
                        <button class="px-4 py-2 rounded-xl text-sm font-semibold text-white {{ $enviarHabilitado ? 'bg-gray-800' : 'bg-gray-400 cursor-not-allowed' }}"
                            ...
                            Enviar a la siguiente secretaria
                        </button>
                        @if(!$procesoEtapa || !$procesoEtapa->recibido)
                        <p class="text-xs text-red-600 mt-1">Debes marcar recibido antes de enviar.</p>
