{{-- Archivo: backend/resources/views/backend/areas/secop-detalle.blade.php | Proposito: Vista documentada para mantenimiento. | @documentado-copilot 2026-04-11 --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <div>
                <div class="breadcrumb-row text-xs text-gray-400 mb-0.5 leading-none">
                    <a href="{{ route('secop.index') }}" class="hover:text-blue-700 transition-colors">SECOP</a>
                    <span class="mx-1">/</span>
                    <span class="breadcrumb-code text-gray-600 font-medium">{{ $proceso->codigo }}</span>
                </div>
                <h1 class="text-lg font-bold text-gray-900">Detalle del proceso — SECOP</h1>
            </div>
    ...
    <div class="p-6 space-y-4">
        ...
        <div class="bg-white rounded-2xl border p-5" style="border-color:#e2e8f0">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm border-t pt-4" style="border-color:#f1f5f9">
                ...
                <div>
                    <p class="text-xs text-gray-400 uppercase tracking-wide mb-0.5">SECOP II</p>
                    <p class="font-medium text-gray-700">{{ $proceso->numero_proceso_secop ?? 'Sin publicar' }}</p>
                </div>
            </div>
        </div>
