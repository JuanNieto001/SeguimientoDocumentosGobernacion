{{-- Archivo: backend/resources/views/backend/secop/show.blade.php | Proposito: Vista documentada para mantenimiento. | @documentado-copilot 2026-04-11 --}}
<x-app-layout>
    <x-slot name="header">
    ...
    <div class="p-6 space-y-4">
        ...
        <div class="bg-white rounded-2xl border p-5" style="border-color:#e2e8f0">
            ...
            <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 text-sm border-t pt-4" style="border-color:#f1f5f9">
                ...
                @if($proceso->numero_proceso_secop)
                <div class="sm:col-span-2">
                    <p class="text-xs text-gray-400 uppercase tracking-wide mb-0.5">N° SECOP II</p>
                    <div class="flex items-center gap-2">
                        <p class="font-bold text-blue-700">{{ $proceso->numero_proceso_secop }}</p>
                        @if($proceso->url_secop)
                        <a href="{{ $proceso->url_secop }}" target="_blank"
        ...
        {{-- Recepción --}}
        <div class="bg-white rounded-2xl border p-5" style="border-color:#e2e8f0">
            <h3 class="text-sm font-semibold text-gray-700 mb-3">Recepción del proceso</h3>
            <form method="POST" action="{{ route('workflow.recibir', $proceso->id) }}">
                @csrf
                <input type="hidden" name="area_role" value="secop">
                ...
                <div class="flex items-center gap-4">
                    <button type="submit"
                        ...
                        {{ $recibido?'Proceso recibido ✓':'Marcar como recibido' }}
                    </button>
        ...
        <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#e2e8f0">
            <div class="px-5 py-4 border-b" style="border-color:#f1f5f9">
                <h3 class="text-sm font-semibold text-gray-700">Publicar en SECOP II</h3>
            </div>
            <div class="p-5">
                @if($proceso->numero_proceso_secop)
                <div class="flex items-center gap-3 p-3 rounded-xl mb-4" style="background:#eff6ff;border:1px solid #bfdbfe">
                    ...
                    <p class="text-sm font-semibold text-blue-700">Publicado: {{ $proceso->numero_proceso_secop }}
                        @if($proceso->fecha_publicacion_secop)
                        — {{ \Carbon\Carbon::parse($proceso->fecha_publicacion_secop)->format('d/m/Y') }}
                        @endif
                    </p>
                </div>
                @endif
                <form method="POST" action="{{ route('secop.publicar', $proceso->id) }}" class="grid sm:grid-cols-2 gap-3">
                    @csrf
                    <div>
                        <label class="block text-xs text-gray-500 mb-1 font-medium">N° Proceso SECOP <span class="text-red-500">*</span></label>
                        <input type="text" name="numero_proceso_secop" value="{{ $proceso->numero_proceso_secop }}"
                    ...
                    <div>
                        <label class="block text-xs text-gray-500 mb-1 font-medium">URL en SECOP II <span class="text-red-500">*</span></label>
                        <input type="url" name="url_secop" value="{{ $proceso->url_secop }}"
                               class="w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                               style="border-color:#e2e8f0" placeholder="https://..." required>
                    </div>
                    <div>
                        <label class="block text-xs text-gray-500 mb-1 font-medium">Fecha publicación <span class="text-red-500">*</span></label>
                        <input type="date" name="fecha_publicacion"
                               value="{{ $proceso->fecha_publicacion_secop ? \Carbon\Carbon::parse($proceso->fecha_publicacion_secop)->format('Y-m-d') : '' }}"
                               class="w-full px-3 py-2.5 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-blue-500"
                               style="border-color:#e2e8f0" required>
                    </div>
                    <div>
                    ...
                    <div class="sm:col-span-2">
                        <button type="submit"
                            ...
                            {{ $proceso->numero_proceso_secop ? 'Actualizar publicación' : 'Publicar en SECOP II' }}
                        </button>
                    </div>
                </form>
            </div>
        ...
        <div class="bg-white rounded-2xl border overflow-hidden" style="border-color:#e2e8f0">
            <div class="px-5 py-4 border-b" style="border-color:#f1f5f9">
                <h3 class="text-sm font-semibold text-gray-700">Etapas de contratación</h3>
            </div>
            <div class="p-5 grid sm:grid-cols-3 gap-3">
                {{-- Registrar Contrato --}}
                <div class="p-4 rounded-xl space-y-3" style="background:#f8fafc;border:1px solid #e2e8f0">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Registrar Contrato</p>
                    <form method="POST" action="{{ route('secop.contrato', $proceso->id) }}">
                        @csrf
                        <div class="space-y-2">
                            <input type="text" name="numero_contrato" placeholder="N° Contrato"
                ...
                {{-- Acta de Inicio --}}
                <div class="p-4 rounded-xl space-y-3" style="background:#f8fafc;border:1px solid #e2e8f0">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Acta de Inicio</p>
                    <form method="POST" action="{{ route('secop.acta.inicio', $proceso->id) }}">
                        @csrf
                        <div class="space-y-2">
                ...
                {{-- Cerrar proceso --}}
                <div class="p-4 rounded-xl space-y-3" style="background:#fefce8;border:1px solid #fde68a">
                    <p class="text-xs font-semibold text-gray-500 uppercase tracking-wide">Cierre del proceso</p>
                    <p class="text-xs text-gray-500">Marca el proceso como finalizado y cerrado en el sistema.</p>
                    <form method="POST" action="{{ route('secop.cerrar', $proceso->id) }}">
                        @csrf
                        <div class="space-y-2">
                            <input type="text" name="observaciones" placeholder="Observaciones de cierre"
                                   class="w-full px-3 py-2 rounded-xl border text-sm focus:outline-none focus:ring-2 focus:ring-yellow-500"
                                   style="border-color:#fde68a">
                            <button type="submit"
                                    onclick="return confirm('¿Cerrar definitivamente el proceso? Esta acción no se puede deshacer.')"
                                    class="w-full px-4 py-2 rounded-xl text-white text-sm font-semibold hover:opacity-90 transition"
            ...
            <div class="p-5 space-y-4">
                <form method="POST" action="{{ route('workflow.files.store', $proceso->id) }}" enctype="multipart/form-data"
                    ...
                    <select name="tipo_archivo" required
                        ...
                        <option value="publicacion_secop">Publicación SECOP</option>
                        <option value="acta_inicio">Acta de inicio</option>
                        <option value="contrato_firmado">Contrato firmado</option>
                        <option value="registro_presupuestal">Registro presupuestal</option>
                ...
                <div class="space-y-2">
                    ...
                    <div class="flex items-center justify-between p-3 rounded-xl" style="background:#f8fafc;border:1px solid #e2e8f0">
                        ...
                        <div class="flex items-center gap-2 shrink-0">
                            <button onclick="window.dispatchEvent(new CustomEvent('abrir-preview', { detail: {{ $ar->id }} }))"
                                    class="p-1.5 rounded-lg text-gray-400 hover:text-green-600 hover:bg-green-50 transition-colors" title="Previsualizar">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                            </button>
                            <a href="{{ route('workflow.files.download', $ar->id) }}"
                               class="p-1.5 rounded-lg text-gray-400 hover:text-blue-600 hover:bg-blue-50 transition-colors" title="Descargar">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                            </a>
                            @if($ar->version > 1)
                            <span class="text-xs px-1.5 py-0.5 rounded-full font-medium" style="background:#dbeafe;color:#2563eb">v{{ $ar->version }}</span>
                            @endif
                            <form method="POST" action="{{ route('workflow.files.destroy', $ar->id) }}" onsubmit="return confirm('¿Eliminar?')">
                                @csrf @method('DELETE')
                                <button class="p-1.5 rounded-lg text-gray-400 hover:text-red-600 hover:bg-red-50 transition-colors" title="Eliminar">
