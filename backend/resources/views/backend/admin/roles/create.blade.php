{{-- Archivo: backend/resources/views/backend/admin/roles/create.blade.php | Proposito: Vista documentada para mantenimiento. | @documentado-copilot 2026-04-11 --}}
<x-app-layout>
    <x-slot name="header">
                ...
                <form method="POST" action="{{ route('admin.roles.store') }}" class="space-y-6">
                    ...
                    <div>
                        ...
                        <select name="dashboard_scope"
                            class="w-full rounded-xl px-3.5 py-2.5 text-sm text-gray-700 outline-none focus:ring-2 focus:ring-green-500 transition-all"
                            style="border:1px solid #e2e8f0">
                            <option value="propios" @selected(old('dashboard_scope') === 'propios')>👤 Propios – Ve solo sus propios procesos</option>
                            <option value="unidad" @selected(old('dashboard_scope') === 'unidad')>📁 Unidad – Ve solo procesos de su unidad</option>
                            <option value="secretaria" @selected(old('dashboard_scope') === 'secretaria')>🏛️ Secretaría – Ve solo procesos de su secretaría</option>
                            <option value="global" @selected(old('dashboard_scope') === 'global')>🌍 Global – Ve todos los procesos del sistema</option>
                        </select>
