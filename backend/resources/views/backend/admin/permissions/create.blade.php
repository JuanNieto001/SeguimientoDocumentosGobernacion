{{-- Archivo: backend/resources/views/backend/admin/permissions/create.blade.php | Proposito: Vista documentada para mantenimiento. | @documentado-copilot 2026-04-11 --}}
<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-4">
            ...
            <div>
                <h1 class="text-lg font-bold text-gray-900 leading-none">Nuevo permiso</h1>
                <p class="text-xs text-gray-400 mt-1">Registrar un permiso en el sistema</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="p-6" style="background:#f1f5f9;min-height:calc(100vh - 65px)">
        <div class="max-w-xl mx-auto">
            <div class="bg-white rounded-2xl p-8" style="border:1px solid #e2e8f0">
                <form method="POST" action="{{ route('admin.permisos.store') }}" class="space-y-5">
                    @csrf
                    <div>
                        <label class="block text-sm font-semibold text-gray-700 mb-1.5">Nombre del permiso</label>
                        <input name="name" value="{{ old('name') }}" placeholder="Ej: procesos.crear"
                        ...
                        <p class="text-xs text-gray-400 mt-1">Convención recomendada: <code>recurso.accion</code> (Ej: <code>procesos.aprobar</code>)</p>
                    </div>
