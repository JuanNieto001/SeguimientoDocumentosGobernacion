...
Route::middleware(['auth', 'usuario.activo'])->group(function () {
    ...
    */
    ...
    /*
    |----------------------------------------------------------------------
    | DASHBOARD BUILDER API - SOLO ADMIN (compatibilidad endpoints actuales)
    |----------------------------------------------------------------------
    */
    Route::middleware('permission:dashboard.builder.access')->prefix('dashboard-builder')->group(function () {
        // Entidades disponibles
        Route::get('/entities',                 [DashboardBuilderController::class, 'entities'])->name('api.dashboard.entities');
        Route::get('/entities/{entity}/fields', [DashboardBuilderController::class, 'fields'])->name('api.dashboard.entity.fields');

        // Queries de widgets
        Route::post('/widget/query',            [DashboardBuilderController::class, 'executeWidget'])->name('api.dashboard.widget.query');
        Route::get('/entity/{entity}/stats',    [DashboardBuilderController::class, 'entityStats'])->name('api.dashboard.entity.stats');
    });
    ...
    */
    Route::middleware('permission:dashboard.view.assigned')->prefix('dashboard-viewer')->group(function () {
        Route::get('/list',                     [DashboardBuilderController::class, 'viewerList'])->name('api.dashboard.viewer.list');
        Route::get('/{id}',                     [DashboardBuilderController::class, 'viewerShow'])->name('api.dashboard.viewer.show');

        // Queries de widgets (solo para dashboards asignados)
        Route::post('/widget/query',            [DashboardBuilderController::class, 'executeWidget'])->name('api.dashboard.viewer.widget.query');
    });

    /*
    |----------------------------------------------------------------------
    | DASHBOARD BUILDER - Motor Visual Dinámico (nueva API)
    |----------------------------------------------------------------------
    */
    Route::prefix('dashboard-builder')->group(function () {
        $ctrl = DashboardBuilderController::class;
